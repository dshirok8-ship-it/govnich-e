  ...
  return (
    <div className="admin">
      <div className="adminHeader">
        <div>
          <h2 className="h2">Админ</h2>
          <div className="muted">
            Данные читаются из <code>/public/data</code>. Здесь мы импортируем и скачиваем JSON-файлы для коммита в репозиторий.
          </div>
        </div>

        <div className="adminHeader__right">
          <div className={`pill ${issueSummary.errors ? 'pill--bad' : issueSummary.warns ? 'pill--warn' : 'pill--ok'}`}>
            Валидатор: ошибок {issueSummary.errors}, предупреждений {issueSummary.warns}
          </div>
        </div>
      </div>
      ...
      {tab === 'import' ? (
        <section className="card">
          <h3 className="card__title">Импорт coverage из CSV</h3>
          <div className="muted">Формат CSV: заголовки <code>companyId,zoneId</code></div>
          ...
          <div style={{ marginTop: 12 }}>
            <div className="muted">
              Preview импортированных строк: <strong>{importState.parsed.length}</strong>
              {importState.applied ? <span className="badge" style={{ marginLeft: 8 }}>применено</span> : null}
            </div>
            ...
            <DataTable
              ...
              emptyText="Загрузи CSV, чтобы увидеть preview"
            />
            {importState.parsed.length > 200 ? <div className="muted">Показаны первые 200 строк.</div> : null}
          </div>
        </section>
      ) : null}
      ...
      {tab === 'coverage' ? (
        <section className="card">
          <h3 className="card__title">Coverage (в памяти)</h3>
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginTop: 8 }}>
            <button className="btn btn--ghost" type="button" onClick={() => downloadJson('coverage.json', coverage)}>
              Скачать coverage.json
            </button>
          </div>
      ...
      {tab === 'validator' ? (
        <section className="card">
          ...
          <div className="muted">Проверки: неизвестные ID, дубли coverage-пар, пустые ссылки, подозрительные URL, структура зон.</div>
